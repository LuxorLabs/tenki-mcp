...
function signDelegation(audience, generation) {
	...
	const header = Buffer.from(
		JSON.stringify({ alg: "HS256", typ: "tenki-mcp-delegation+jwt" }),
	).toString("base64url");
...
const identity = http.createServer((req, res) => {
	...
	req.on("end", () => {
		const presentedToken = JSON.parse(body).accessToken;
		if (
			presentedToken !== sourceToken &&
			presentedToken !== refreshedSourceToken
		) {
...
try {
	...
	for (const path of [
		...
		if (response.status !== 404) {
			throw new Error(`MCP server still owns OAuth provider route ${path}`);
		}
	}
	...
	if (refreshedDelegatedRequests < 1) {
		throw new Error("the MCP session did not adopt its refreshed API delegation");
	}
	if (identityExchanges < 2) {
		throw new Error("MCP access tokens were not exchanged through Tenki Identity");
	}
	if (
