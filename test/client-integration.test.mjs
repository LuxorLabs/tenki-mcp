 ...
 *                                       71 tools with valid, unique, described object schemas.
 ...
 *   • startup-and-auth-contract      — no key → degraded auth-only mode;
...
const NO_CREDENTIAL_STDERR = "starting in unauthenticated mode";
...
async function waitExit(rec, ms = 4000) {
	if (exited(rec.child))
		return { code: rec.child.exitCode, signal: rec.child.signalCode };
	return Promise.race([
...
function assertClean401(text, key) {
	must(
		!text.includes(key),
		`SECURITY: token value echoed in whoami error output`,
	);
	if (/\b401\b|unauthenticated|unauthorized/i.test(text)) return; // clean, attributable auth error
	if (isDataPlaneOutage({ message: text }))
		throw new Error(`control-plane unreachable: ${text}`); // → SKIP
	throw new Error(
		`unexpected whoami response (no 401/auth error): ${text.slice(0, 200)}`,
	);
}
...
try {
	...
	await h.check(
		`handshake: serverInfo {name:tenki, version:${VERSION}} + capabilities.tools.listChanged`,
		async () => {
			const info = h.client.getServerVersion();
			must(
				info?.name === "tenki",
				`serverInfo.name=${info?.name} (want 'tenki')`,
			);
			must(
				info?.version === VERSION,
				`serverInfo.version=${info?.version} (want '${VERSION}')`,
			);
			const caps = h.client.getServerCapabilities();
			must(
				caps?.tools?.listChanged === true,
				`capabilities.tools.listChanged=${JSON.stringify(caps?.tools)}`,
			);
		},
	);

	await h.check(
		`tools/list: exactly ${h.tools.length} unique tenki_* tools, all described with object schemas`,
		async () => {
			must(
				h.tools.length === 71,
				`advertised ${h.tools.length} tools (documented surface = 71 (70 workflow tools + tenki_auth_status); any drift is a regression)`,
			);
			const names = h.tools.map((t) => t.name);
			const dupes = names.filter((n, i) => names.indexOf(n) !== i);
			must(
				dupes.length === 0,
				`duplicate tool names: ${JSON.stringify([...new Set(dupes)])}`,
			);
			for (const t of h.tools) {
				...
				must(
					...
					`tool '${t.name}' inputSchema.type=${s?.type} (want 'object')`,
				);
				const props = s.properties ?? {};
				for (const req of s.required ?? []) {
					must(
						Object.prototype.hasOwnProperty.call(props, req),
						`tool '${t.name}' requires '${req}' absent from properties`,
					);
				}
			}
		},
	...
	await h.check(
		`handshake: raw stdio spawn negotiates latest protocol, advertises ${h.tools.length} tools <2s offline`,
		async () => {
			must(latest.init, "no initialize response");
			must(
				!latest.init.error,
				`initialize errored: ${JSON.stringify(latest.init.error)}`,
			);
			must(
				latest.init.result?.protocolVersion === LATEST_PROTOCOL_VERSION,
				`echoed protocol ${latest.init.result?.protocolVersion} (want ${LATEST_PROTOCOL_VERSION})`,
			);
			const si = latest.init.result?.serverInfo;
			must(
				si?.name === "tenki" && si?.version === VERSION,
				`serverInfo=${JSON.stringify(si)}`,
			);
			must(
				latest.init.result?.capabilities?.tools?.listChanged === true,
				`capabilities=${JSON.stringify(latest.init.result?.capabilities)}`,
			);
			must(
				latest.list?.result?.tools?.length === h.tools.length,
				`raw tools/list=${latest.list?.result?.tools?.length} (want ${h.tools.length})`,
			);
			must(
				latest.initMs < 2000,
				`handshake took ${latest.initMs}ms (want <2000ms offline)`,
			);
		},
	);

	await h.check(
		"stdout purity: every stdout frame after handshake+tools/list is valid JSON-RPC; banner only on stderr",
		async () => {
			must(
				latest.rec.lines.length >= 2,
				`expected ≥2 stdout frames (init+tools), got ${latest.rec.lines.length}`,
			);
			for (const m of latest.rec.messages) {
				must(
					!m.__unparsed,
					`non-JSON stray write on stdout: ${String(m.__unparsed).slice(0, 120)}`,
				);
				must(
					m.jsonrpc === "2.0",
					`stdout frame not JSON-RPC 2.0: ${JSON.stringify(m).slice(0, 120)}`,
				);
			}
			must(
				!latest.rec.stdout.includes(BANNER),
				"banner leaked onto stdout (must be stderr only)",
			);
			must(
				latest.rec.stderr.includes(BANNER),
	...
	await h.check(
		`handshake: older protocol (${OLDER_PROTOCOL}) negotiates without disconnect, still ${h.tools.length} tools`,
		async () => {
			const { rec, init, list } = await rawHandshake(OLDER_PROTOCOL);
			must(init, "no initialize response for older protocol");
			must(
				!init.error,
				`older-protocol initialize errored: ${JSON.stringify(init.error)}`,
			);
			must(
				init.result?.protocolVersion === OLDER_PROTOCOL,
				`echoed ${init.result?.protocolVersion} (want ${OLDER_PROTOCOL} — server should honor a supported older version)`,
			);
			must(
				list?.result?.tools?.length === h.tools.length,
				`older-protocol tools/list=${list?.result?.tools?.length} (want ${h.tools.length})`,
			);
			must(
				!exited(rec.child),
				"server disconnected after older-protocol handshake",
			);
			rec.child.stdin.end();
		},
	);
	...
	await h.check(
		"shutdown: idle EOF → 0 bytes stdout, banner on stderr, exit 0 within 2s",
		async () => {
			const rec = launch({ ...baseEnv(), TENKI_API_KEY: "tk_offline_dummy" });
			...
			must(
				ex.code === 0,
				`idle-EOF exit code=${ex.code} signal=${ex.signal} (want 0)`,
			);
			must(
				rec.stdout.length === 0,
				`idle server wrote ${rec.stdout.length} bytes to stdout (want 0)`,
			);
			must(
				rec.stderr.includes(BANNER),
				"banner missing from stderr on idle launch",
			);
	...
	await h.check(
		"shutdown: client.close() reaps the child fast with no orphan pid",
		async () => {
			const transport = new StdioClientTransport({
				command: process.execPath,
				args: [SERVER],
				env: { ...baseEnv(), TENKI_API_KEY: "tk_offline_dummy" },
				stderr: "pipe",
			});
			...
			must(
				!alive,
				`orphaned child pid ${pid} still alive after client.close()`,
			);
		},
	);

	await h.check(
		"shutdown: SIGTERM on a running server exits within 2s, no orphan",
		async () => {
			const rec = launch({ ...baseEnv(), TENKI_API_KEY: "tk_offline_dummy" });
			must(
				await waitForStderr(rec, BANNER, 3000),
				"server never printed its startup banner",
			);
	...
	await h.check(
		"auth: no key → degraded auth-only mode, then clean EOF shutdown",
		async () => {
			const rec = launch(baseEnv()); // neither TENKI_API_KEY nor TENKI_AUTH_TOKEN
			must(
				await waitForStderr(rec, BANNER, 3000),
				"no-key launch never printed the banner",
			);
			must(!exited(rec.child), "no-key launch exited instead of running");
			must(
				rec.stderr.includes(NO_CREDENTIAL_STDERR),
				`stderr missing degraded-mode guidance; got: ${JSON.stringify(rec.stderr.trim())}`,
			);
			must(
				rec.stdout.length === 0,
				`no-key launch wrote ${rec.stdout.length} bytes before a request (want 0)`,
			);
			rec.child.stdin.end();
			const ex = await waitExit(rec, 3000);
			must(!ex.timedOut, "no-key launch did not exit after stdin EOF");
			must(
				ex.code === 0,
				`no-key exit code=${ex.code} signal=${ex.signal} (want 0)`,
			);
		},
	);

	await h.check(
		"auth: TENKI_AUTH_TOKEN fallback boots (banner on stderr, alive, 0 bytes stdout)",
		async () => {
			const rec = launch({
				...baseEnv(),
				TENKI_AUTH_TOKEN: "tk_offline_dummy_authtoken",
			}); // fallback env, no TENKI_API_KEY
			must(
				await waitForStderr(rec, BANNER, 3000),
				"fallback launch never printed the banner",
			);
			must(!exited(rec.child), "fallback launch exited instead of running");
			must(
				rec.stdout.length === 0,
				`fallback launch wrote ${rec.stdout.length} bytes to stdout (want 0)`,
			);
			rec.child.stdin.end();
		},
	);
	...
	await h.checkData(
		"auth: bogus tk_ key → clean 401 via whoami (Bearer branch), token not echoed",
		async () => {
			const r = await spawnClientWhoami("tk_deadbeef_not_a_real_key_000111222");
			must(
				!r.threw,
				`whoami threw instead of returning an isError result: ${r.text.slice(0, 160)}`,
			);
			must(
				r.isErr,
				`whoami on a bad key returned success (isError=false): ${r.text.slice(0, 160)}`,
			);
	...
	await h.checkData(
		"auth: bogus ory_st_ key → clean 401 via whoami (X-Session-Token branch), token not echoed",
		async () => {
			const r = await spawnClientWhoami(
				"ory_st_deadbeefnotarealsessiontoken000",
			);
			must(
				!r.threw,
				`whoami threw instead of returning an isError result: ${r.text.slice(0, 160)}`,
	...
	await h.checkData(
		"auth: bogus no-prefix key → clean 401 via whoami (Cookie branch), token not echoed",
		async () => {
	...
	await h.check(
		"no orphan pids: every spawned server child was reaped",
		async () => {
			const orphans = [];
			for (const rec of spawned) {
				if (!exited(rec.child)) {
					...
					if (ex.timedOut && !exited(rec.child))
						orphans.push(`raw:${rec.child.pid}`);
				}
			}
			for (const pid of clientPids) {
				try {
					process.kill(pid, 0);
					orphans.push(`client:${pid}`);
				} catch {
					/* reaped */
				}
			}
			must(
				orphans.length === 0,
				`orphaned server pids survived the suite: ${orphans.join(", ")}`,
			);
		},
	);
} catch (e) {
