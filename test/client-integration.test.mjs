 ...
 *                                       70 tools with valid, unique, described object schemas.
...
try {
	...
	await h.check(`tools/list: exactly ${h.tools.length} unique tenki_* tools, all described with object schemas`, async () => {
		must(h.tools.length === 70, `advertised ${h.tools.length} tools (documented surface = 70 (69 workflow tools + tenki_auth_status); any drift is a regression)`);
