...
function loadToken() {
	...
	try {
		return (
			readFileSync(`${homedir()}/.config/tenki/config.yaml`, "utf8").match(
				/^auth_token:\s*(.+)$/m,
			)?.[1] ?? ""
		).trim();
	} catch {
...
const child = spawn(process.execPath, [SERVER], {
	env: {
		...process.env,
		TENKI_MCP_TRANSPORT: "http",
		PORT: String(PORT),
		TENKI_MCP_HTTP_TOKEN: HTTP_TOKEN,
		TENKI_API_KEY: token,
	},
...
try {
	...
	check(
		"server started on loopback with auth",
		banner.includes("127.0.0.1") && banner.includes("bearer auth required"),
		banner.trim(),
	);
	...
	check(
		"rejects request with no bearer token (401)",
		noAuth.status === 401,
		`got ${noAuth.status}`,
	);
	...
	const badHost = await rawPost(
		{ Authorization: `Bearer ${HTTP_TOKEN}`, Host: "evil.example.com" },
		initBody,
	);
	check(
		"rejects forged Host header (DNS-rebinding guard)",
		badHost.code === 403,
		`got ${badHost.code}`,
	);
