...
console.log(`owner: ${owner.ownerType}/${owner.ownerId?.slice(0,8)} ws=${ws?.slice(0,8)}\n`);
...
let volId;
const cv = await check("CreateVolume", () => client.control("CreateVolume", { workspaceId: ws, name: "mcp-verify-vol", sizeBytes: 1_048_576 }));
...
try {
	const created = await client.control("CreateSession", { cpuCores: 1, memoryMb: 1024, maxDuration: "600s", idleTimeoutMinutes: 5, allowInbound: true, allowOutbound: true,
		...(owner.ownerType ? { ownerType: owner.ownerType } : {}), ...(owner.ownerId ? { ownerId: owner.ownerId } : {}), ...(ws ? { workspaceId: ws } : {}) });
	...
	// previews — bodies as the tools send them (slug; projects no longer exist in the API)
	await check("ExposePort", () => client.control("ExposePort", { sessionId: sid, port: 8080 }));
	await check("CreatePreviewUrl", () => client.control("CreatePreviewUrl", { sessionId: sid, port: 8080, slug: "mcp-verify-preview" }));
	await check("OpenPreview", () => client.control("OpenPreview", { sessionId: sid, port: 8080 }));
	await check("ListPreviewUrls", () => client.control("ListPreviewUrls", { workspaceId: ws, sessionId: sid }));
