		...
		"tenki_get_upload_url",
		{
			...
			inputSchema: z.object({
				...
				path: pathSchema.describe(
					"Destination path in the sandbox, e.g. /home/tenki/data.bin",
				),
				content_type: z
					.string()
					.optional()
					.describe("MIME type of the upload, e.g. application/octet-stream."),
			}),
		},
		...
		"tenki_get_download_url",
		{
			...
			inputSchema: z.object({
				...
				artifact_id: z
					.string()
					.describe(
						"Artifact id to download (e.g. a command's stdout/stderr artifact).",
					),
			}),
		},
