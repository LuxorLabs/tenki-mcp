		...
		"tenki_get_workspace_usage",
		{
			...
			inputSchema: z.object({
				workspace_id: z
					...
					.optional()
					.describe(
						"Workspace to report on. Omit to use the API key's first workspace.",
					),
			}),
		},
		async ({ workspace_id }) => {
		...
		"tenki_get_workspace_settings",
		{
			...
			inputSchema: z.object({
				workspace_id: z
					...
					.optional()
					.describe(
						"Workspace to read. Omit to use the API key's first workspace.",
					),
			}),
		},
		async ({ workspace_id }) => {
		...
		"tenki_update_workspace_settings",
		{
			...
			inputSchema: z.object({
				workspace_id: z
					...
					.optional()
					.describe(
						"Workspace to update. Omit to use the API key's first workspace.",
					),
				pause_retention_days: z
					...
					.optional()
					.describe(
						"How long a paused sandbox's snapshot is kept before it becomes unresumable.",
					),
				clear_pause_retention: z
					.boolean()
					.optional()
					.describe(
						"Remove the pause-retention override (use the platform default).",
					),
				...
				clear_snapshot_retention: z
					...
					.optional()
					.describe(
						"Remove the snapshot-retention override, i.e. keep snapshots indefinitely.",
					),
				max_snapshots: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum snapshots in the workspace."),
				max_templates: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum templates in the workspace."),
				max_volumes: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum volumes in the workspace."),
				max_total_bytes: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum combined snapshot + volume storage in bytes."),
				max_concurrent_sessions: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum simultaneously active sandboxes."),
				max_sticky_sessions: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum sticky sandboxes."),
				max_preview_urls: z
					.number()
					.int()
					.positive()
					.optional()
					.describe("Maximum preview URLs in the workspace."),
			}),
		},
		async (a) => {
		...
		"tenki_get_snapshot_retention_settings",
		{
			...
			inputSchema: z.object({
				workspace_id: z
					.string()
					.optional()
					.describe("Workspace (defaults to the key's first workspace)."),
			}),
		},
		async ({ workspace_id }) => {
			const workspaceId =
			...
			return ok(
				await client.control("GetWorkspaceSnapshotRetentionSettings", {
					...(workspaceId ? { workspaceId } : {}),
				}),
			);
		},
		...
		"tenki_update_snapshot_retention_settings",
		{
			...
			inputSchema: z.object({
				retention_days: z
					...
					.optional()
					.describe(
						"Days to retain snapshots before automatic cleanup. Omit and pass clear_retention to keep them indefinitely.",
					),
				clear_retention: z
					.boolean()
					.optional()
					.describe(
						"Remove the retention period so snapshots are kept indefinitely. Mutually exclusive with retention_days.",
					),
				workspace_id: z
					.string()
					.optional()
			...
			return ok(
				await client.control("UpdateWorkspaceSnapshotRetentionSettings", {
					...
					...(retention_days !== undefined
						? { snapshotRetention: `${retention_days * 86400}s` }
						: {}),
