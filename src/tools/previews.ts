...
export function registerPreviews(server: McpServer, client: TenkiClient): void {
		...
		"tenki_unexpose_port",
		{
			...
			inputSchema: z.object({
				session_id: sessionIdSchema.describe(
					"The sandbox session whose port to unexpose.",
				),
				port: portSchema.describe(
					"The TCP port inside the sandbox to unexpose (1-65535).",
				),
			}),
		},
		async ({ session_id, port }) =>
			ok(await client.control("UnexposePort", { sessionId: session_id, port })),
	);
	...
	server.registerTool(
		"tenki_create_preview_url",
		{
			...
			inputSchema: z.object({
				session_id: sessionIdSchema.describe(
					"The sandbox session serving the port.",
				),
				port: portSchema.describe(
					"The TCP port inside the sandbox to create a preview URL for (1-65535).",
				),
				slug: slugSchema,
				expires_at: z
					.string()
					.optional()
					.describe(
						"Optional RFC-3339 timestamp at which the preview URL auto-expires. Omit to keep it until the sandbox ends.",
					),
			}),
		},
	...
	server.registerTool(
		"tenki_open_preview",
		{
			...
			inputSchema: z.object({
				...
				port: portSchema.describe(
					"The TCP port inside the sandbox to open a preview for (1-65535).",
				),
				expires_at: z
					.string()
					.optional()
					.describe(
						"Optional RFC-3339 timestamp at which the preview auto-expires. Omit to keep it until the sandbox ends.",
					),
			}),
		},
	...
	server.registerTool(
		"tenki_list_preview_urls",
		{
			...
			inputSchema: z.object({
				session_id: sessionIdSchema
					.optional()
					.describe(
						"Keep only preview URLs bound to this sandbox. Applied client-side to the fetched page.",
					),
				workspace_id: z
					.string()
					.optional()
				...
				page_size: z
					...
					.max(100)
					.optional()
					.describe("Rows per page (server default 20, max 100)."),
				page_token: z
					.string()
					.optional()
		...
		async ({ session_id, workspace_id, page_size, page_token }) => {
			...
			const rows: any[] = Array.isArray(resp.previewUrls)
				? resp.previewUrls
				: [];
			const filtered = session_id
				? rows.filter((r) => r?.sessionId === session_id)
				: rows;
			return ok({
				previewUrls: filtered,
				...(resp.nextPageToken ? { nextPageToken: resp.nextPageToken } : {}),
				...(session_id
					? { filteredClientSide: true, fetchedOnThisPage: rows.length }
					: {}),
			});
		},
	);
	...
	server.registerTool(
		...
		{
			description: "Fetch a specific preview URL's details by id.",
			inputSchema: z.object({
				preview_url_id: z.string().describe("The preview URL id."),
			}),
		},
		async ({ preview_url_id }) =>
			ok(
				await client.control("GetPreviewUrl", { previewUrlId: preview_url_id }),
			),
	);
	...
	server.registerTool(
		...
		{
			description: "Delete a preview URL by id, taking it permanently offline.",
			inputSchema: z.object({
		...
		async ({ preview_url_id }) =>
			ok(
				await client.control("DeletePreviewUrl", {
		...
		"tenki_touch_preview",
		{
			...
			inputSchema: z.object({
				preview_token: z
					.string()
					.describe(
						"The preview token (from create_preview_url / open_preview).",
					),
			}),
		},
		async ({ preview_token }) =>
			ok(await client.control("TouchPreview", { previewToken: preview_token })),
	);
	...
	server.registerTool(
		...
		async ({ preview_url_id }) =>
			ok(
				await client.control("UnbindPreviewUrl", {
					previewUrlId: preview_url_id,
				}),
			),
	);
	...
	server.registerTool(
		...
		async ({ token }) =>
			ok(await client.control("ResolvePreviewToken", { token })),
	);
}
