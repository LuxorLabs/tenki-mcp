...
export function registerFiles(server: McpServer, client: TenkiClient): void {
	server.registerTool(
		"tenki_read_file",
		{
			...
			inputSchema: z.object({ session_id: sessionIdSchema, path: pathSchema }),
		},
		async ({ session_id, path }) =>
			ok({ path, content: await client.readTextFile(session_id, path) }),
	);
	...
	server.registerTool(
		...
		async ({ session_id, path, content }) =>
			ok(await client.writeTextFile(session_id, path, content)),
	);
	...
	server.registerTool(
		"tenki_list_files",
		{
			...
			inputSchema: z.object({
				...
				include_hidden: z
					...
					.optional()
					.describe(
						"Include dot-prefixed entries (default true). The data plane omits them unless asked, which hides .git/.env from a listing.",
					),
			}),
		},
		async ({ session_id, path, include_hidden }) => {
			...
			const resp = await client.data(session_id, "List", {
				path,
