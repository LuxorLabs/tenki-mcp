...
export function registerFiles(server: McpServer, client: TenkiClient): void {
	server.tool(
		...
		{ session_id: sessionIdSchema, path: z.string() },
	...
	server.tool(
		...
		{ session_id: sessionIdSchema, path: z.string(), content: z.string() },
	...
	server.tool(
		...
		{ session_id: sessionIdSchema, path: z.string().describe("Directory path, e.g. /home/tenki") },
