...
export function registerSandboxes(
			...
			inputSchema: z
				.object({
					...
					disk_size_gb: z
						...
						.positive()
						.optional()
						.describe("Disk in GB (default 5)."),
					max_duration_seconds: z
						.number()
						.int()
						.positive()
						.optional()
						.describe("Hard lifetime cap in seconds."),
					idle_timeout_minutes: z
						.number()
						.int()
						.positive()
						.optional()
						.describe("Reap after N idle minutes."),
					clone_repo_url: z
						.string()
						.optional()
						.describe("Git URL to clone into the sandbox on boot."),
					allow_outbound: z
						.boolean()
						.optional()
						.describe("Allow outbound networking (off by default)."),
					allow_inbound: z
						.boolean()
						.optional()
						.describe("Allow inbound networking (off by default)."),
					...
					wait_ready: z
						...
						.optional()
						.describe(
							"Poll until the sandbox is RUNNING before returning (default true).",
						),
				})
				.strict(),
		},
		async (a) => {
			...
			const body: Record<string, unknown> = {
				...
				...(a.max_duration_seconds
					? { maxDuration: `${a.max_duration_seconds}s` }
					: {}),
				...(a.idle_timeout_minutes
					? { idleTimeoutMinutes: a.idle_timeout_minutes }
					: {}),
			...
			const dataPlaneEndpoint =
				resp.dataPlaneEndpoint ?? resp.data_plane_endpoint;
	...
	server.registerTool(
		"tenki_get_sandbox",
		{
			description: "Fetch a sandbox's current state and metadata.",
			inputSchema: z.object({ session_id: sessionIdSchema }),
		},
		async ({ session_id }) =>
			ok(await client.control("GetSession", { sessionId: session_id })),
	);

	server.registerTool(
		"tenki_list_sandboxes",
		{
			description: "List sandboxes for the workspace.",
	...
	server.registerTool(
		...
		async ({ session_id }) =>
			ok(await client.control("TerminateSession", { sessionId: session_id })),
	);
	...
	server.registerTool(
		...
		async ({ session_id }) =>
			ok(await client.control("PauseSession", { sessionId: session_id })),
	);

	server.registerTool(
		"tenki_resume_sandbox",
		{
			description: "Resume a previously paused sandbox.",
			inputSchema: z.object({ session_id: sessionIdSchema }),
		},
		async ({ session_id }) =>
			ok(await client.control("ResumeSession", { sessionId: session_id })),
	);
}
